...
## Описание проектов
В одном решении присутсвуют два проекта. Первый проект `cuda_convolution_UI` является основной программой (для открытия в IDE нужно установить Microsoft .NET Framework 4.7.2). Данному проекту для запуска требуется файл `cudaFanc64.dll`, который можно получить путём сборки второго проекта - `kernal_to_dll`. 
